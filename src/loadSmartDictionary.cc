...
bwaidx_t* loadIndex(std::string path,std::string out_fold)
{
	string file = get_file_from_path(path);
	vector < string > nameFileWhitExtension = split_vargeno(file, '.');
	string nameFile = out_fold+"/" + nameFileWhitExtension[0];
 ...
 */
void complete_smart_dict(std::string path_smartDict, std::string path_reassembly_snp, std::string snpTemp_filename, std::string  out_p)
{
	/*
	...
	std::cerr << path_reassembly_snp << std::endl;
	bwaidx_t* index_reassembly = loadIndex(path_reassembly_snp.c_str(),out_p);
