...
void createIndex(std::string path,std::string outPath)
{
	string file = get_file_from_path(path);
	vector < string > nameFileWhitExtension = split_vargeno(file, '.');
	string nameFile = outPath+"/" + nameFileWhitExtension[0];
...
int main(const int argc, const char *argv[])
{
	...
	if (inputUserComand.compare("create_incomplete_smartSnpDictionary") == 0)
	{
		...
		snp_dict_filename = path_output +'/' + prefix + "SmartDict" + name;
	...
	else if (inputUserComand.compare("reassembly") == 0)
	{
		...
		//Per default
		string snpTemp_filename = outputFolder+"/" + prefix + "Temp.txt";

		//
		string path_smartDict = outputFolder+"/" + prefix + "SmartDict" + name;

		//Devo aggiungere una estensione per simmetria
		std::string path_reass = outputFolder+"/" + prefix + "Reassembly" + name + ".txt";
	...
	else if (inputUserComand.compare("complete_smartSnpDictionary") == 0)
	{
		...
		std::string snpDictionary_filename = outputFolder+"/" + prefix + "SmartDict" + name;
		std::cerr << snpDictionary_filename << std::endl;
		//Path containing the k-mer in the literal format
		std::string snpTemp_filename = outputFolder+"/" + prefix +"Temp" + ".txt";
		std::cerr << snpTemp_filename << std::endl;
		//Path containing the re-assembly indexation
		std::string path_index_reassembly_snp = outputFolder+"/" + prefix + "Reassembly" + name ;
		std::cerr << path_index_reassembly_snp << std::endl;
		std::cerr << "\nStarting complete SNP dictionary..." << std::endl;

		//Complete smart dictionary
		complete_smart_dict(snpDictionary_filename, path_index_reassembly_snp, snpTemp_filename,outputFolder);
